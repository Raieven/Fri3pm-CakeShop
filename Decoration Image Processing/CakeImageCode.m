...
for i = 1:3%:size(detectedCakeBlocks,1)
    %cakeOrientation = 0;
    convOrientation = 0;
...
%         sprintf('yellowCakeShape: %d\n', yellowCakeShape)
    ...
    %CW = positve
    %CCW = negative
    cakeBlockOrder(blockOrderCounter,7) = cakeOrientation - convOrientation;
...
for i = 1:size(detectedConvBlocksCentres,1)
    if (detectedConvBlocksCentres(i,1) ~= -100)
        leftOverBlocks(j,:) = detectedConvBlocksCentres(i,:);
        j=j+1;
    end
end
...
%     Square = 6
    ...
    elseif ((BP.Area >= 545 && BP.Area <= 809) && (BP.Solidity >= 0.50 && BP.Solidity <= 0.70 ) && ...
        ...
        cakeShape =  4;
    elseif ((BP.Area >= 454 && BP.Area <= 919) && (BP.Solidity >= 0.47 && BP.Solidity <= 0.4 ) && ...
...
%     Square = 6
    ...
    elseif ((CB.Area >= 750 && CB.Area <= 816) && (CB.Solidity >= 0.9 && CB.Solidity <= 0.97) && ...
        ...
        disp("No Shape Appears to Match with The Cake");
        convShape = 7;
    end
end
...
%     Square = 6
    if ((CB.Area >= 800 && CB.Area <= 1011) && (CB.Solidity >= 0.92 && CB.Solidity <= 0.99 ) && ...
            ...
            (CB.MinorAxisLength >= 31 && CB.MinorAxisLength <= 34) && (CB.MajorAxisLength >= 31 && CB.MajorAxisLength <= 38)) %(CB.Eccentricity >= 0.15 && CB.Eccentricity <= 0.32) 
        convShape = 1;
    elseif ((CB.Area >= 566 && CB.Area <= 809) && (CB.Solidity >= 0.84 && CB.Solidity <= 0.95 ) && ...
            (CB.Perimeter >= 99 && CB.Perimeter <= 142) && (CB.EquivDiameter >= 25 && CB.EquivDiameter <= 33) &&...
            (CB.MinorAxisLength >= 25 && CB.MinorAxisLength <= 33) && (CB.MajorAxisLength >= 26 && CB.MajorAxisLength <= 34)) %(CB.Eccentricity >= 0.09 && CB.Eccentricity <= 0.4) 
        convShape = 2;
...
%     yellow = 4
    ...
    %           green
                ...
                CB = regionprops(shapes, {'Area', 'Eccentricity', 'EquivDiameter', 'EulerNumber', 'MajorAxisLength', 'MinorAxisLength', 'Orientation', 'Perimeter', 'Solidity'});
                idx = find([CB.Area] > 220);
                CB = CB(idx);
...
function angle = getCakeOrientation(block)
    ...
    stats = regionprops(IbwBlock, 'Extrema');
