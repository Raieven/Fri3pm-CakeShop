...
%     conveyor_imshow = imshow(conveyor_image, 'Parent', conveyor_cam_ax);
    ...
    conveyor_out_imshow = imshow(default_conveyor_image, 'Parent', conveyor_out_ax);
    hold on;
    

%     [ink_out, Traj, Draw] = ink.update(default_table_image);
%     [normal, bold] = parse_letters(aj);
%     detectedCakeBlocks = [];
...
%     handle01 = plot(0, 0, 'b.', 'Parent', table_out_ax);

    

    
%     Your_handle.XData = Data(:,1);
%     Your_hanfle.YData = Data(:,2);
...
%          plot(hd01, RR, CC, style, 'Markersize', Boldness);
%         plot(hd02, RR, CC, style, 'Markersize', Boldness);
